...
def su4(circuit: tc.Circuit, qubit_1:int, qubit_2:int, theta: jax.Array) -> tc.Circuit:
    ...
    circuit.rx(qubit_2, theta=theta[0, 0])
    circuit.ry(qubit_2, theta=theta[0, 1])
    circuit.rz(qubit_2, theta=theta[0, 2])
    circuit.rxx(qubit_1, qubit_2, theta=theta[1, 0])
    circuit.ryy(qubit_1, qubit_2, theta=theta[1, 1])
    circuit.rzz(qubit_1, qubit_2, theta=theta[1, 2])
