...
from solvers import UVQESolver, LOCCSolver
...
def main():
    ...
    results = {}
    for i in range(11):
        result = uvqe_solver.train(500, 0.1 * i)
        results[i] = result
    
    # Prepare data for plotting
    perturb = [0.1 * i for i in results.keys()]
    exact_energy = [results[i]['exact_energy'] for i in results.keys()]
    ...
    out_dir = os.path.join("results", uvqe_solver.exp_name, uvqe_solver.timestamp_str)
    # Create the plot
    plt.style.use('seaborn-v0_8-darkgrid')
    plt.plot(perturb, exact_energy, label='Exact Energy')
    plt.plot(perturb, final_min_energy, label='Final Min Energy')
    plt.plot(perturb, final_min_energy_locc, label='Final Min Energy (LOCC)')
